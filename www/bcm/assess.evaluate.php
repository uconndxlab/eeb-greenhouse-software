<?php
#function assess.evaluate($codeno)
#{
...
$codeno=201300013;  ##################  TEMPORARY TESTING
...
#	*** DEDUCT 5 points for Importance = 'Redundant'
$x = $result['importance']; 
if ($x=='Redundant') {
	$points = $points - 5;
	$evalcriteria = $evalcriteria.'<li>Redundant Specimen = (-5)';
}
...
#	*** Score 3 Points for each class usage
...
$points = $points + $num;
if ($num > 0 ) $evalcriteria = $evalcriteria.'<li>Class Use: (3pts each) = '.$num;
...
#$sth->execute();
#$tempresult = $sth->fetch(PDO::FETCH_ASSOC);
#if ($num > 0 ) $evalcriteria = $evalcriteria.'<li>Descretionary Points: = '.$num.' = '.mysql_result($sql_result,0,"ev_disc_re");

### Update database record with new criteria
$sql = 'update gh_inv set eval_rank='.($points+$num).', e_criteria="'.$evalcriteria.'",e_tx_rank='.$tx_temp.',e_cl_rank='.$cl_temp.' where codeno='.$codeno;
$sth = $db->prepare($sql);
$sth->execute();
$tempresult = $sth->fetch(PDO::FETCH_ASSOC);

# Calculate new ranking
$sql = 'select codeno,eval_rank,space_req from gh_inv where projnum="GEN_COLL" order by eval_rank desc';
$i=0;
$coll_rank=9999;
$space_req = 0;
foreach($db->query($sql) as $row) {
	if ($coll_rank==9999) $space_req = $space_req + $row['space_req'];
	if ($row["codeno"]==$codeno) $coll_rank = $i + 1;
	$i++;
}
...
echo $codeno.': '.$result['latin_name'].chr(10); 
echo 'Pts: '.$points.' '.$evalcriteria.chr(10);
echo 'Rank: '.$coll_rank,': '.'Space Req:'.$space_req.chr(10);

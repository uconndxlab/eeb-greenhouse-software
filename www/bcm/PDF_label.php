...
### Formats with 'CM' after have been tweaked from default formats to avoid 'creep'
    var $_Avery_Labels = array(
        ...
        '5161CM' => array('paper-size'=>'letter',    'metric'=>'mm',    'marginLeft'=>0.967,    'marginTop'=>5,        'NX'=>2,    'NY'=>10,    'SpaceX'=>3.967,    'SpaceY'=>0,    'width'=>101.6,        'height'=>25.4,        'font-size'=>8),
        '5161H' => array('paper-size'=>'letter',    'metric'=>'mm',    'marginLeft'=>0.967,    'marginTop'=>10.7,        'NX'=>1,    'NY'=>10,    'SpaceX'=>3.967,    'SpaceY'=>0,    'width'=>101.6,        'height'=>25.4,        'font-size'=>8),
...
###        '5161' => array('paper-size'=>'letter',    'metric'=>'mm',    'marginLeft'=>0.967,    'marginTop'=>14,        'NX'=>2,    'NY'=>10,    'SpaceX'=>3.967,    'SpaceY'=>0,    'width'=>101.6,        'height'=>25.4,        'font-size'=>8),
...
##########
// Experimental Line by line extraction and placement
    function Add_FLabel($codeno,$latin_name,$commonname,$family,$cntry_orig,$habitat) {
        $this->_COUNTX++;
        if ($this->_COUNTX == $this->_X_Number) {
            // Row full, we start a new one
            $this->_COUNTX=0;
        ...
        $_PosY = $this->_Margin_Top + $this->_COUNTY*($this->_Height+$this->_Y_Space) + $this->_Padding;
        $this->SetXY($_PosX, $_PosY);
	// add line by line formatting here
	$this->SetFont('Arial','B',12);
	$this->Cell(75,0,$latin_name,0,0);

#	$this->SetXY($_PosX+80, $_PosY);
#	$this->SetFont('Arial','B',10);
#	$this->Cell(20,0,'  UConn#');

	$this->SetXY($_PosX, $_PosY+4);
	$this->SetFont('Arial','',9);
	$this->Cell(0,0,$commonname);

#	$this->SetXY($_PosX+80, $_PosY+4);
#	$this->SetFont('Arial','B',10);
#	$this->Cell(20,0,$codeno);

	$this->SetXY($_PosX, $_PosY+8);
	$this->SetFont('Arial','',9);
	$this->Cell(0,0,$cntry_orig);

	$this->SetXY($_PosX, $_PosY+12);
	$this->SetFont('Arial','',9);
	$this->Cell(0,0,$habitat);

	$this->SetXY($_PosX, $_PosY+16);
	$this->SetFont('Arial','B',12);
	$this->Cell(20,0,$codeno);

#	$this->SetXY($_PosX+80, $_PosY+16);
	$this->SetXY($_PosX+75, $_PosY+16);
	$this->SetFont('Arial','B',12);
	$this->Cell(20,0,$family,0,0,'R');
	}
			


// End of Experimental Label formatter...
##########
